...
const TransferScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  const { bankAccounts, updateBankBalance, addTransaction, calculateCashBalance } = useFinance();
  const { theme, isDarkMode } = useTheme();

  const [amount, setAmount] = useState<string>('');
  const [fromType, setFromType] = useState<any>('cash'); // 'cash' or 'bank'
  const [toType, setToType] = useState<any>('bank'); // 'cash' or 'bank'
  ...
  const renderBankSelection = (type: any$3selectedBankId, setSelectedBankId, errorKey) => (
