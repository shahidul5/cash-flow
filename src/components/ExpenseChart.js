...
const ExpenseChart = ({ transactions }) => {
  ...
  const expenses = transactions ? transactions.filter(t => t && t.type === 'expense') : [];
  ...
  const chartData = Object.keys(expensesByCategory).map(category => {
    ...
    return {
      ...
      color: categoryColors[categoryKey] || '#607D8B',
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.totalContainer}>
        ...
        <Text style={[styles.totalAmount, { color: theme.text }]}>{formatCurrency(totalExpenses)}</Text>
      </View>
    </View>
  );
};
